...
def _read_jsonl_ids(path: str, key="_id"):
    out = []
...
def _load_qrels_tsv(path: str) -> pd.DataFrame:
    # Robust: handles header/no-header
...
def load_beir_jsonl(
    ...
    query_embed_path: str | None = None,
    ...
    if embed_path and os.path.exists(embed_path):
        ...
        print("⚙️ building passage embeddings (no cache)", flush=True)
        texts = []
        passage_ids = []
    ...
    # Query embeddings (optional cache)
    if query_embed_path and os.path.exists(query_embed_path):
        ...
        queries_emb = encoder.encode(
            query_texts, batch_size=query_batch_size,
    ...
    meta = {
        ...
        "counts": {"passages": len(passage_ids), "queries": len(query_ids), "pos_qrels": int(len(pos))},
    }
    print(f"✅ loaded: passages={len(passage_ids)} queries={len(query_ids)} split={qrels_split}", flush=True)
