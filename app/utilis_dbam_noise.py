...
def maybe_flip_bool(arr, p, rng):
    """
    Flip boolean entries with probability p.
    Models Paul's counting-noise idea:
      1 -> 0 with prob p
      0 -> 1 with prob p
    """
    if p <= 0:
        return arr
    flips = rng.random(arr.shape) < p
    return np.logical_xor(arr, flips)


# --- DBAM core ---


def dbam_direct(q_code, base_q, alpha, m, g, count_noise_p=0.0, noise_rng=None):
    ...
    if count_noise_p > 0:
        rng = noise_rng if noise_rng is not None else np.random.default_rng()
        ub = maybe_flip_bool(ub, count_noise_p, rng)
        lb = maybe_flip_bool(lb, count_noise_p, rng)

    return ub.sum(axis=1) + lb.sum(axis=1)


def dbam_dual(
    q_code, base_q, base_dual, alpha, m, g, levels, count_noise_p=0.0, noise_rng=None
):
    bg = base_q.reshape(-1, g, m)
    ...
    if count_noise_p > 0:
        rng = noise_rng if noise_rng is not None else np.random.default_rng()
        ub_orig = maybe_flip_bool(ub_orig, count_noise_p, rng)
        ub_dual = maybe_flip_bool(ub_dual, count_noise_p, rng)
...
def build_global_negatives_once(
    query_ids_sample,
    query_to_gt,
    pipeline_data,
    K_final,
    ...
    for qi, qid in tqdm(
        ...
        if len(negs) == 0:
            ...
            negs = rng.choice(
                pipeline_data["passage_ids_sample"], size=K_final, replace=False
            ).tolist()
...
def train_W_param(
    ...
    all_negs = build_global_negatives_once(
        ...
        MS_INFER=MS_INFER,
        STAGES_INFER=STAGES_INFER,
    )
    # map to indices (pid_to_idx keys are strings)
    neg_indices = [
        pid_to_idx[str(n)] if not isinstance(n, str) else pid_to_idx[n]
        for n in all_negs
        if (str(n) in pid_to_idx)
    ]
    ...
    for ep in range(epochs):
        print(
            f"\n⚙️ [Epoch {ep+1}/{epochs}] Training with prebuilt negatives...",
            flush=True,
        )
        ...
        for i in pbar:
            ...
            loss = info_nce_loss_with_prenorm(
                q_proj, pos_vecs, X_neg_norm, temperature=0.07
            )
        ...
        print(
            f"[train] epoch {ep+1}/{epochs} avg_loss={running/max(1,steps_per_epoch):.4f}",
            flush=True,
        )
    ...
    torch.save(
        {"state_dict": adapter.state_dict(), "in_dim": D, "bottleneck": 64},
        adapter_path,
    )
...
def build_pipeline_with_W(
    ...
    if not os.path.exists(adapter_path):
        ...
        if require_adapter:
            ...
            print(
                f"⚠️ {msg} -> Proceeding WITHOUT adapter (queries will be unchanged).",
                flush=True,
            )
            adapter = None
    else:
        ...
        adapter = Adapter(in_dim=ckpt["in_dim"], bottleneck=ckpt["bottleneck"]).to(
            device
        )
    ...
    pipeline_data = {
        ...
        "centroids_dual": np.ascontiguousarray(
            (levels - 1) - centroids_q, dtype=np.int32
        ),
        "W": W_np,
...
def build_pipeline_baseline(
    slug, embeddings, queries_emb_sample, passage_ids_sample, bits_sq, nlist
):
    ...
    base_q = np.ascontiguousarray(
        quantize_np_perdim_minmax(X, x_min, x_max, levels), dtype=np.int32
    )
    centroids_q = np.ascontiguousarray(
        quantize_np_perdim_minmax(centroids, x_min, x_max, levels), dtype=np.int32
    )
    queries_q = np.ascontiguousarray(
        quantize_np_perdim_minmax(Q, q_min, q_max, levels), dtype=np.int32
    )
...
def retrieve_pipeline(q_vec, q_code, params, data):
    ...
    # --- NEW: inference-time counting noise only ---
    count_noise_p = float(params.get("count_noise_p", 0.0))
    noise_rng = params.get("noise_rng", None)

    if s1 == "ivf":
        selected_clusters = data["ivf_flat"].quantizer.search(q_vec[None, :], nprobe)[
            1
        ][0]
    elif s1 == "dual":
        sc = dbam_dual(
            ...
            g1,
            data["levels"],
            count_noise_p=count_noise_p,
            noise_rng=noise_rng,
        )
        selected_clusters = np.argsort(-sc)[:nprobe]
    elif s1 == "ivf_int4":
        diffs = data["centroids_q"] - q_code
    ...
    if s2 == "ivf":
        ...
        diffs = data["base_q"][candidate_indices] - q_code
        order = np.argsort(np.einsum("ij,ij->i", diffs, diffs))
    else:
        order = np.argsort(
            -dbam_dual(
                q_code,
                data["base_q"][candidate_indices],
                data["base_dual"][candidate_indices],
                a2,
                m2,
                g2,
                data["levels"],
    ...
    if s3 == "ivf":
        ...
        order = np.argsort(
            -dbam_dual(
                q_code,
                data["base_q"][stage2_candidates],
                data["base_dual"][stage2_candidates],
                a3,
                m3,
                g3,
                data["levels"],
...
def load_or_build_pipeline_for_adapter(
    trial_slug,
    embeddings,
    queries_emb_sample,
    passage_ids_sample,
    ...
    W_dummy = np.eye(
        embeddings.shape[1], dtype=np.float32
    )  # W isn’t used; identity is fine
...
def run_and_evaluate(config, output_dir, pipeline_data, query_ids_sample, query_to_gt):
    results_list = []
    s1, s2, s3 = (
        config["stage_methods"]["s1"],
        config["stage_methods"]["s2"],
        config["stage_methods"]["s3"],
    )
    k_final_values = config.get("k_final_values", [100])
    K_final = max(k_final_values)

    count_noise_p = float(config.get("count_noise_p", 0.0))
    noise_seed = int(config.get("noise_seed", 123))

    def _print_row(tag, row_dict):
        fields = (
            ["nprobe", "K2"]
            + [f"R@{k}" for k in k_final_values]
            + [f"MRR@{k}" for k in k_final_values]
        )
        s = " | ".join([f"{f}={row_dict.get(f, '')}" for f in fields])
        print(f"[{tag}] {s}", flush=True)

    def _retrieve_all(params, desc):
        retrieved_all = []
        for i in tqdm(range(len(pipeline_data["queries_float"])), desc=desc):
            params_i = dict(params)
            if count_noise_p > 0:
                params_i["noise_rng"] = np.random.default_rng(noise_seed + i)
            retrieved = retrieve_pipeline(
                pipeline_data["queries_float"][i],
                pipeline_data["queries_q"][i],
                params_i,
                pipeline_data,
            )
    ...
    if "k2_sweep_values" in config:
        ...
        for k2 in config["k2_sweep_values"]:
            ...
            retrieved_all = _retrieve_all(params, desc=f"Stage sweep K2={k2}")
            rec = compute_recall(
                query_ids_sample, retrieved_all, query_to_gt, k_final_values
            )
            mrr = compute_mrr(
                query_ids_sample, retrieved_all, query_to_gt, k_final_values
            )
            hit = compute_hit_at_k(
                query_ids_sample, retrieved_all, query_to_gt, k_final_values
            )
            row.update(hit)
            row.update(rec)
        ...
        for nprobe in config["nprobe_sweep_values"]:
            ...
            retrieved_all = _retrieve_all(params, desc=f"nprobe={nprobe}")
            rec = compute_recall(
                query_ids_sample, retrieved_all, query_to_gt, k_final_values
            )
            mrr = compute_mrr(
...
def quick_eval_for_pipeline_ivf(
    pipeline_data,
    trial_slug,
    SELECT_NPROBE,
    ALPHAS_INFER,
    ...
    noise_tag = f"noise{int(round(COUNT_NOISE * 100))}"
    cfg = {
        "experiment_name": f"quick_nprobe{SELECT_NPROBE}_{trial_slug}_ivf_{noise_tag}",
        ...
        "count_noise_p": COUNT_NOISE,
        "noise_seed": NOISE_SEED,
    }
    df = run_and_evaluate(
        cfg, results_dir, pipeline_data, query_ids_sample, query_to_gt
    )
    row = df.iloc[0].to_dict()
    score = (row["R@10"], row["MRR@10"])
    print(
        f"[select] {trial_slug} -> noise={COUNT_NOISE:.3f} | R@10={row['R@10']:.4f}, MRR@10={row['MRR@10']:.4f}",
        flush=True,
    )
    return score, row


def quick_eval_for_pipeline_ivf_int4(
    pipeline_data,
    trial_slug,
    SELECT_NPROBE,
    ALPHAS_INFER,
    ...
    noise_tag = f"noise{int(round(COUNT_NOISE * 100))}"
    cfg = {
        "experiment_name": f"quick_nprobe{SELECT_NPROBE}_{trial_slug}_ivf_int4_{noise_tag}",
        "nprobe_sweep_values": [SELECT_NPROBE],
...
def quick_eval_for_pipeline_ddd(
    pipeline_data,
    trial_slug,
    SELECT_NPROBE,
    ALPHAS_INFER,
    ...
    noise_tag = f"noise{int(round(COUNT_NOISE * 100))}"
    cfg = {
        "experiment_name": f"quick_nprobe{SELECT_NPROBE}_{trial_slug}_ddd_{noise_tag}",
        "nprobe_sweep_values": [SELECT_NPROBE],
...
def quick_eval_for_pipeline_dual(
    pipeline_data,
    trial_slug,
    SELECT_NPROBE,
    ALPHAS_INFER,
    ...
    noise_tag = f"noise{int(round(COUNT_NOISE * 100))}"
    cfg = {
        "experiment_name": f"quick_nprobe{SELECT_NPROBE}_{trial_slug}_dual_{noise_tag}",
        "nprobe_sweep_values": [SELECT_NPROBE],
...
def quick_eval_for_pipeline_direct(
    pipeline_data,
    trial_slug,
    SELECT_NPROBE,
    ALPHAS_INFER,
    ...
    noise_tag = f"noise{int(round(COUNT_NOISE * 100))}"
    cfg = {
        "experiment_name": f"quick_nprobe{SELECT_NPROBE}_{trial_slug}_direct_{noise_tag}",
        "nprobe_sweep_values": [SELECT_NPROBE],
